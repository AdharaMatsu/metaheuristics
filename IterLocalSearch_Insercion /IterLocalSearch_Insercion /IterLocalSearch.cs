namespace IterLocalSearch_Insercion;
...
public static class IterLocalSearch
{
    ...
    public static void Run(string filename)
    {
        ...
        while (iteraciones > 0)
        {
            ...
            {
                solucionInicial = Insercion(solucionVecina);
            }
    ...
    private static int[] Insercion(int[] solucion)
    {
        int[] nuevaSolucion = solucion.ToArray();

        int nodo_rnd = rnd.Next(1, solucion.Length); // Nodo a mover
        int nuevaPosicion = rnd.Next(1, solucion.Length); // Nueva posición

        // Extraer el nodo seleccionado
        int nodo = nuevaSolucion[nodo_rnd];

        // Eliminar el nodo de su posición actual
        List<int> lista = nuevaSolucion.ToList();
        lista.RemoveAt(nodo_rnd);

        // Insertar el nodo en la nueva posición
        lista.Insert(nuevaPosicion, nodo);
    ...
    private static int[][] Create(string filename)
    {
        ...
        for (i = 0; i < n; i++)
        {
            for (j = i + 1; j < n; j++)
            {
                grafo[i][j] = Euclidean(distancias[i].Item1, distancias[i].Item2, distancias[j].Item1,
                    distancias[j].Item2);
                grafo[j][i] = grafo[i][j];
            }
        }
