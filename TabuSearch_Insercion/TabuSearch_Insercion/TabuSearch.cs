...
namespace TabuSearch_Insercion;
    ...
    private static int[] generarVecina(int[] solucion)
{
    ...
    // Elegir aleatoriamente entre intercambio, inversión o inserción
    int operacion = rnd.Next(0, 3);  // 0 para intercambio, 1 para inversión, 2 para inserción
    ...
    else if (operacion == 2) // Operación de inserción
    {
        // Elegir un nodo aleatorio para mover
        int ciudadA = rnd.Next(1, solucion.Length); // Excluye el primer nodo (punto de inicio)

        // Elegir una posición aleatoria donde insertar el nodo
        int ciudadB = rnd.Next(1, solucion.Length);

        // Asegurarse de que las posiciones sean diferentes y no estén en la lista tabú
        while (ciudadA == ciudadB || 
               ListaTabu.Contains((ciudadA, ciudadB)) || 
               ListaTabu.Contains((ciudadB, ciudadA)))
        {
        ...
        // Realizar la operación de inserción: quitar ciudadA y colocarla en la posición de ciudadB
        int ciudad = solVecina[ciudadA];
        
        // Eliminar el nodo en la posición ciudadA
        List<int> tempSolucion = solVecina.ToList();
        tempSolucion.RemoveAt(ciudadA);
        
        // Insertar el nodo en la nueva posición ciudadB
        tempSolucion.Insert(ciudadB, ciudad);
        
        // Convertir la lista nuevamente a un arreglo
        solVecina = tempSolucion.ToArray();

        // Agregar el par de ciudades involucradas en la inserción a la lista tabú
        ListaTabu.Add((ciudadA, ciudadB));
        if (ListaTabu.Count > tamañoTabu) // Limitar el tamaño de la lista tabú
        {
            ListaTabu.RemoveAt(0);
